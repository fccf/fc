...
contains
  ...
  !=============================================================================
  subroutine msh_get_ein(this, eptr, eind)
    class(msh_file), intent(in) :: this
    integer, allocatable, intent(out) :: eptr(:), eind(:)
    integer :: ne, enn, ie
    ...
    eptr(1) = 1
    do ie = 1, ne
      eptr(ie+1) = eptr(ie) + enn
      eind(eptr(ie):eptr(ie+1)-1) = this%elist(ie)%eni(1:enn)
    enddo

  end subroutine msh_get_ein
  !=============================================================================
  ...
  !=============================================================================
  subroutine msh_get_fin(this, fptr, find)
    class(msh_file), intent(in) :: this
    integer, allocatable, intent(out) :: fptr(:), find(:)
    integer :: nf, fnn, i
    ...
    allocate(fptr(nf+1))
    allocate(find(nf*fnn))

    fptr(1) = 1
    do i = 1, nf
      fptr(i+1) = fptr(i) + fnn
      find(fptr(i):fptr(i+1)-1) = this%elist(i)%eni(1:fnn)
    enddo
  end subroutine msh_get_fin
  !=============================================================================
